...
public class RotationMatrix {
    ...
    // utilizes the approx. for d/dt(f(x1, y1, z1)) = f_x(x1, y1, z1) * dx/dt + f_y(x1, y1, z1) *
    // dy/dt + f_z(x1, y1, z1) * dz/dt
    ...
    private double findElevationIncrement() {
        ...
        if (yForward) {
            if (Math.abs(incremented[0][1]) < AutoConstants.toZeroBound) {
                // this could honestly cause problems with the derivative approx. bc returning 0 is a mere
                // convenient guess, not mathematically proven
                return 0.0;
            } else {
                x = 1;
