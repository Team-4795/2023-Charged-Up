...
public class Rollerbar extends SubsystemBase {
    ...
    private Rollerbar(RollerbarIO io) {
        this.io = io;
    ...
    public boolean safeToMove(double position) {
        if(position > RollerbarConstants.kWristRetractedBoundary){
            return true;
        } else if (position > RollerbarConstants.kArmBoundary && Wrist.getInstance().getPosition() > WristConstants.rollerbarSetpoint){
            return true;
        }
        return false;
    }
